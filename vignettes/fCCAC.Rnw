...
Once regions of interest, such as peaks, exons, UTRs, etc. are obtained, bigwig data of the mapped reads are the necessesary too for using \Biocpkg{fCCAC}, and can be obtained using \Biocpkg{rtracklayer}. \\
...
\begin{small}
<<fCCACexample, fig.show='asis', include=TRUE, fig.width=1, fig.height=3, background='#C9C6C6'>>=
options(width=60);
...
\begin{small}
<<heatmapPlot, fig.show='asis', include=TRUE, fig.width=1, fig.height=1.5, background='#C9C6C6'>>=
options(width=60)
if (.Platform$OS.type == "windows") { print("...rtracklayer is unable to read bigWig format files in Windows...") }
